...
The automatic upgrade procedure does not work as expected from {product-title} 4.11 and earlier. A summary of the upgrade procedure is as follows:

. Delete the previously installed Operator version for example 4.11. Ensure that the namespace and the `metallb` custom resource are not removed.
