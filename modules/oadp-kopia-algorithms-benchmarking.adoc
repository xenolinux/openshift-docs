...
. Configure the `must-gather` pod as shown in the following example. Make sure you are using the `oadp-mustgather` image for {oadp-short} version 1.3 and later. See the following example pod configuration:
+
[source,yaml]
----
apiVersion: v1
