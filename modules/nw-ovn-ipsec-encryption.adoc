...
The IPsec mode used is _Transport mode_, a mode that encrypts end-to-end communication by adding an encapsulated Security Payload (ESP) header to the IP header of original packet and encrypts the packet data. {product-title} does not currently use and support IPsec _Tunnel mode_ for pod-to-pod communication.
