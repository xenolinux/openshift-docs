...
The AWS Load Balancer Operator supports the Kubernetes service resources of type `LoadBalancer` by using the Network Load Balancer (NLB) with the `instance` and `ip` target type.
