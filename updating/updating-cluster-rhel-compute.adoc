...
* If you run an Operator or you have configured any application with the Pod Disruption Budget, you might experience a block during the upgrade process. The `PodDisruptionBudget` with `minAvailable: 1`  may block the eviction process while nodes are drained to apply pending MachineConfigs. If several nodes are rebooted, all the pods may run on only one node, and the `PodDisruptionBudget` can prevent the node drain.
